class BrowserActions {
     ...
     *            Whether it is Displayed and Enabled both
     */
    ...
    async validateSorting(sColumnName, sDtype) {
        try {
            ...
            console.log(
                "********DESCENDING SORTING BEGINS WITH COLUMN " +
                sColumnName +
                "**********"
            );
            browser.sleep(10000);
            await actions.clickOn(
                ...
                "Click on Column " + sColumnName + " for Sort in Descending"
            );
            // await browser.pause();
    ...
    async performSearchWithColumnValue(sColumnName) {
        try {
            ...
            //Enter Search Values
            await actions.type(element(by.css('input[type=\'search\']')), sRandomValue, sRandomValue + ' Value has been typed');
            //wait for some time
